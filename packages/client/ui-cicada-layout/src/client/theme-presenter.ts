 ...
 * for the token palette, alias-token overrides as inline CSS variables on
 * body, the content font-size axis, and one presenter-owned
 * `meta[name="theme-color"]`. Pure DOM writes, no React; the presenter only
 * ever retracts what it wrote itself. Same contract as the native ui-layout
 * presenter (the cicada frame owns the root slot and must keep theme
 * projection alive).
 */
...
export class CicadaThemePresenter {
   ...
   * palette attribute from `active.colorScheme`, publish the content font-size
   * axis, then replace the previously applied token variables with
   * `active.tokens`. Browser theme-color metadata follows the computed body
   * background after those writes.
