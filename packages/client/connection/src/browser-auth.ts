  ...
  authorizeIndex(req: ConnectionIndexRequest, res: ConnectionIndexResponse): boolean {
    ...
    if (tokens.length > 0) {
      ...
      if (req.method === 'GET' && url.pathname === '/' && tokens.length === 1
        ...
        // Keep non-token query parameters (M1c: ?layout=workspace) across the
        // token exchange redirect; the token itself is intentionally dropped.
        const clean = new URL('/', 'http://dsh.invalid')
        for (const [key, value] of url.searchParams) {
          if (key === TOKEN_QUERY) continue
          clean.searchParams.append(key, value)
        }
        const redirectPath = clean.pathname === '/' && clean.search === '' ? '/' : clean.pathname + clean.search
        res.writeHead(303, {
