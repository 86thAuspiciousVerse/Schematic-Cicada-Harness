...
async function run(ctx: Context, task: string, io: HeadlessIo): Promise<void> {
  ...
  // that DOES configure one joins its default preset through this setup, which
  // is the only point that runs before the agent is published
  // (@deepseek-ai/dsh-agent-presets README, "Composing a child agent").
  // Read structurally: the roster is optional and this bundle must not depend
  // on its package just to compose with it.
  const presets = ctx.get('agentPresets') as { mount(agentCtx: Context): Promise<unknown> } | undefined
