...
describe('headless runner preset join', () => {
  it('joins the composed preset roster for its session', async () => {
    const test = await bench({
      afterPrompt: async (session, message) => {
        await Promise.resolve()
        appendTurn(session, 1, message, 'joined answer', true)
      },
    })
    const mounted: unknown[] = []
    // A deployment that composes the roster (the cicada profile does) mounts its
    // default preset through the create setup; without this the session would
    // run on the global layer alone and lose every preset-scoped tool.
