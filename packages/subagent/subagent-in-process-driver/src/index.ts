...
/**
 * Append the one-shot descriptor during the unpublished creation window.
 *
 * The descriptor is the durable identity used by scoped hosts (for example
 * Cicada's producer tool installer) when they observe `agent/created`.  It
 * therefore must exist before publication, rather than being deferred until
 * the first `pre-step` event.  This also keeps creation observers from seeing
 * a child whose role is temporarily indeterminate.
 */
