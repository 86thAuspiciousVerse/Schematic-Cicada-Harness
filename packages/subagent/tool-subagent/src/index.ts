...
export interface Config {
  ...
  /**
   * Model-facing description for this instance's tool. Omission uses the
   * generic delegation wording — which is identical for every instance, so a
   * deployment with several role-specific rows (`spawn_knowledge`,
   * `spawn_datasheet`, …) MUST set this: otherwise the model cannot tell the
   * roles apart (measured 2026-09-13: three rows, one description).
   */
  ...
  const install = (runtimeCtx: Context, modelSelectionPolicy: ModelSelectionPolicy | undefined): void => {
    ...
    const mount = (subagentProvider: SubagentProvider): void => {
      ...
      const disposeTool = runtimeCtx.tools.register(defineTool({
        name: toolName,
        description: (config.toolDescription !== undefined && config.toolDescription !== ''
